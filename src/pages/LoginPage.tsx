...
import { ShieldCheck, Loader2, ArrowRight, AlertCircle, RefreshCcw, User, ArrowLeft, LogOut, KeyRound, Mail, Eye, EyeOff } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const [loginMode, setLoginMode] = useState<'GOOGLE' | 'MANUAL'>('MANUAL');
  ...
  const handleAuthError = (err: any) => {
    ...
    if (err.code === 'auth/popup-blocked') {
      ...
      message = "Log masuk manual (Email/Password) belum diaktifkan di Firebase Console.";
      setDebugInfo("Sila aktifkan 'Email/Password' provider di Firebase Console > Authentication > Sign-in method.");
    } else {
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20">
          <div className="gov-gradient p-12 text-center text-white relative">
            <motion.div
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ delay: 0.2 }}
              className="bg-white p-5 rounded-[2rem] shadow-xl inline-block mb-6"
            >
              <img src="https://i.postimg.cc/T3NqjCYM/logo-penangpng.png" alt="Jata Pulau Pinang" className="h-24 w-24" />
            </motion.div>
            <h1 className="text-4xl font-bold mb-2 tracking-tight">Sistem e-Kariah</h1>
            <p className="text-blue-100/80 text-sm font-medium uppercase tracking-widest">Portal Pentadbiran & Pengurusan</p>
          </div>
            ...
            {user ? (
                ...
                {loginMode === 'GOOGLE' ? (
                  ...
                  <form onSubmit={handleManualLogin} className="space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button 
                        ...
                        className="text-[10px] font-bold text-gov-blue hover:underline uppercase tracking-widest"
                      >
                        LUPA KATA LALUAN?
                      </button>
                    </div>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="••••••••"
                        className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-4 pl-12 pr-12 text-sm focus:border-gov-blue focus:bg-white outline-none transition-all"
                      />
                      <button
                        type="button"
                        onClick={() => setShowPassword(!showPassword)}
                        className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-gov-blue transition-colors"
                      >
                  ...
                  >
                    {isLoggingIn || authLoading ? (
                      ...
                      <>LOG MASUK PENTADBIR <ArrowRight size={18} /></>
                    )}
                  </button>
                </form>
              )}
              ...
              <div className="relative py-4">
                ...
                <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-widest">
                  <span className="bg-white px-4 text-slate-400">Atau Log Masuk Melalui</span>
                </div>
              </div>
